...
const NavMobile = () => {
  const {handleIsLoginModal, handleIsLeftNav, isLeftNav, setIsLeftNav} = useModal();
    ...
    <nav className="px-4 py-2.5 lg:hidden bg-white z-40 shadow-sm">
      <div className="flex justify-between items-center flex-wrap text-primary-text">
        <div className="cursor-pointer" onClick={handleIsLeftNav}>
          <div className="w-7 h-[1.5px] bg-primary-text mb-2" />
        ...
        {!isAuthenticated ? (<div>
          <IoPersonCircleOutline className="text-[23px] hover:text-custom3 transition-all duration-300 delay-150 ease-in-out cursor-pointer" onClick={handleIsLoginModal}/>
        </div>) : (<AuthenticatedNavProfile />)}
