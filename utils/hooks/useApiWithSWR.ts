...
interface ApiWithSWRConfig extends SWRConfiguration<PaginatedApiResponse, Error> {
  defaultLimit?: number;
}
...
const fetcher = async (url: string) => {
  try {
    ...
    if (Array.isArray(errorDetails) && errorDetails.length > 0) {
      ...
      console.error(`Error Type: ${errorType}, Location: ${location}, Message: ${errorMessage}`);
    }
...
const useApiWithSWR = (
  ...
  const { data, error } = useSWR<PaginatedApiResponse, Error>(
    url,
    fetcher,
    {
